...
export default function App() {
  ...
  const applyPreset = useCallback((key) => { const p = PRESETS[key]; if (!p) return; setInputs({ ...p.data, data_types: { ...p.data.data_types }, tiers: { ...p.data.tiers } }); setFlagships([]); }, []);
  ...
  const renderStep = () => {
    switch (kioskStep) {
      ...
      case 5: return <ResultsPage r={r} galenMigrationCost={galenMigrationCost} galenAnnualCost={galenAnnualCost} onReset={() => { setKioskStep(0); setFlagships([]); }} />;
      default: return null;
    }
  };
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: C.bg, width: W, height: H, color: C.text, lineHeight: 1.55, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {/* Nav */}
      <NavButtons step={kioskStep} totalSteps={KIOSK_STEPS.length}
        onBack={() => setKioskStep(p => p - 1)}
        onNext={() => setKioskStep(p => p + 1)}
        onCalculate={() => setKioskStep(5)} />
    </div>
  );
}
