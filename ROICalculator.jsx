...
// ── Known EPR/PAS systems (UK & Ireland) ──
const KNOWN_SYSTEMS = [
  { label: "Epic", cost: 8000000, type: "EPR" },
  { label: "Oracle Health (Cerner) Millennium", cost: 5000000, type: "EPR" },
  { label: "Dedalus Lorenzo", cost: 3500000, type: "PAS/EPR" },
  { label: "TPP SystmOne", cost: 4000000, type: "EPR" },
  { label: "System C Careflow", cost: 800000, type: "PAS/EPR" },
  { label: "MEDITECH Expanse", cost: 2500000, type: "EPR" },
  { label: "InterSystems TrakCare", cost: 2000000, type: "EPR" },
  { label: "Allscripts / Altera", cost: 1200000, type: "EPR" },
  { label: "IMS MAXIMS", cost: 800000, type: "PAS" },
  { label: "Civica Paris", cost: 500000, type: "PAS" },
  { label: "Cambio COSMIC", cost: 1000000, type: "EPR" },
  { label: "Nervecentre", cost: 1000000, type: "e-Obs" },
  { label: "CliniSys Winpath LIMS", cost: 400000, type: "LIMS" },
  { label: "Varian ARIA", cost: 2500000, type: "Oncology" },
  { label: "EMIS Health", cost: 350000, type: "Clinical" },
  { label: "System C Badgernet", cost: 200000, type: "Maternity/Neonatal" },
  { label: "BigHand", cost: 250000, type: "Dictation" },
  { label: "WellSky (JAC) Pharmacy", cost: 300000, type: "Pharmacy" },
];
...
export default function ROICalculator() {
  ...
  const addFlagship = useCallback((sys) => { const s = sys || {}; setFlagships(p => [...p, {name:s.label||"", cost:s.cost||250000, retire:true, known:s.label||""}]); }, []);
  ...
  const ref1=useRef(null),ref2=useRef(null),refFlag=useRef(null),ref3=useRef(null),refCta=useRef(null),thinkingRef=useRef(null),calcPageRef=useRef(null);
  const scrollTo=ref=>setTimeout(()=>ref.current?.scrollIntoView({behavior:"smooth",block:"start"}),120);
  useEffect(() => { if (thinking && thinkingRef.current) thinkingRef.current.scrollIntoView({behavior:"smooth",block:"center"}); }, [thinking]);
  useEffect(() => { if (page === "calculating" && calcPageRef.current) setTimeout(()=>calcPageRef.current?.scrollIntoView({behavior:"smooth",block:"center"}),100); }, [page]);
  ...
  const handleOrgCountChange = useCallback((v) => {
    const prevOrgs = inputs.org_count;
    update("org_count", v);
    if (v !== prevOrgs && prevOrgs > 0) {
      const r = v / prevOrgs;
      setInputs(p => ({...p, org_count: v,
        bed_count: Math.round(p.bed_count * r / 10) * 10,
        tiers: {
          enterprise: Math.max(1, Math.round(p.tiers.enterprise * r)),
          departmental: Math.round(p.tiers.departmental * r),
          niche: Math.round(p.tiers.niche * r),
        },
      }));
    ...
    setOrgTiersList(prev => {
      const next = [...prev];
      while (next.length < v) next.push({enterprise:1, departmental:3, niche:5});
      return next.slice(0, v);
    });
    touch(1);
  }, [update, touch, inputs.org_count]);
  ...
  const applyPreset=useCallback(key=>{
    ...
    setActivePreset(key);resetOv();setThinking(true);
        ...
        reader.onload = async () => {
          ...
          try {
            await fetch(EMAIL_ENDPOINT, {
              ...
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                to: leadForm.email, name: leadForm.name, org: leadForm.org || "",
                role: leadForm.role || "", filename: pdf.filename, pdf_base64: base64,
                scenario: am==="CONSERVATIVE"?"Conservative":am==="STRETCH"?"Stretch":"Expected",
                annual: r.annual, total3: r.total3,
              }),
            });
  ...
  const inputPage = <div>
    {thinking && (
      <div ref={thinkingRef} style={{position:"fixed",inset:0,background:"rgba(27,36,51,0.55)",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(3px)"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {orgBedsList.map((beds, i) => (
                <div key={i} className="roi-org" style={{display:"flex",alignItems:"center",gap:12,padding:"8px 14px",background:C.bg,borderRadius:8,border:`1px solid ${C.borderLight}`}}>
                  ...
                  <input type="range" min={0} max={Math.max(4000, beds + 500)} step={10} value={beds}
      ...
      {!perOrgSystems ? (
        <div>
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            {[
              {tier:"enterprise",label:"Enterprise systems",color:C.accent,bg:C.accentPale,br:C.border,hint:"e.g. legacy EPR, PAS, large clinical suites",max:Math.max(10, inputs.tiers.enterprise + 2),tip:"Large clinical platforms, e.g. legacy EPR/PAS, major departmental suites. High per-system cost."},
              {tier:"departmental",label:"Departmental systems",color:"#1a7a7a",bg:"#eef9f7",br:"#c0e8e2",hint:"e.g. theatres, lab, ED, maternity, radiology",max:Math.max(30, inputs.tiers.departmental + 5),tip:"Mid-tier clinical systems serving one department or specialty. Moderate running costs."},
              {tier:"niche",label:"Standalone systems",color:"#3d8a8a",bg:C.tealPale,br:`${C.teal}22`,hint:"e.g. document stores, scanned notes, specialty databases",max:Math.max(100, inputs.tiers.niche + 10),tip:"Small self-contained tools, e.g. single-function databases, document stores, scanned notes. Low individual cost but they add up."},
            ].map(t=>{
            ...
            {orgTiersList.map((org, oi) => (
              <div key={oi} className="roi-org" style={{padding:"12px 16px",background:C.bg,borderRadius:10,border:`1px solid ${C.borderLight}`}}>
                ...
                <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                  {[
                    {tier:"enterprise",label:"Enterprise",color:C.accent,max:Math.max(5, org.enterprise + 2)},
                    {tier:"departmental",label:"Departmental",color:"#1a7a7a",max:Math.max(15, org.departmental + 3)},
                    {tier:"niche",label:"Standalone",color:"#3d8a8a",max:Math.max(50, org.niche + 5)},
                  ].map(t=>(
    ...
    {step<2?<ContinuePrompt onClick={()=>goTo(2,refFlag)} label="Next: named systems"/>:<LockedConnector/>}

    {/* STEP 3 - Named high-cost systems */}
    <div ref={refFlag} style={{animation:step>=2?"fade-in .4s ease":"none"}}><Card dimmed={step<2}>
      <div style={{fontSize:12,fontWeight:600,color:C.textMuted,textTransform:"uppercase",letterSpacing:".05em",marginBottom:4,display:"flex",alignItems:"center",gap:8}}>
        ...
        Named high-cost systems
        {flagships.length > 0 && <span style={{marginLeft:"auto",display:"inline-flex",alignItems:"center",justifyContent:"center",minWidth:24,height:24,borderRadius:12,padding:"0 8px",background:C.accent,color:"#fff",fontSize:13,fontWeight:800,lineHeight:1}}>{flagships.length}</span>}
      </div>
      <p style={{fontSize:13,color:C.textMuted,margin:"0 0 16px"}}>Do you know the specific legacy systems you plan to archive or decommission? Adding them with actual contract values gives a more accurate estimate.</p>

      <div style={{marginBottom:16}}>
        <div style={{fontSize:13,fontWeight:700,color:C.text,marginBottom:8}}>Select a known system to add</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {KNOWN_SYSTEMS.slice(0, 10).map(sys => (
            <button type="button" key={sys.label} onClick={()=>{addFlagship(sys);touch(3);}} className="roi-preset" style={{width:"100%",padding:"10px 12px",textAlign:"left",border:`1px solid ${C.border}`,borderRadius:14,background:"#fff",cursor:"pointer"}}>
              <div style={{fontSize:13,fontWeight:700,color:C.text}}>{sys.label}</div>
              <div style={{fontSize:11,color:C.textMuted,marginTop:1}}>{sys.type} - typical ~{fmtK(sys.cost)}/yr</div>
            </button>
          ))}
        </div>
        <details style={{marginTop:8}}>
          <summary style={{fontSize:13,fontWeight:600,color:C.accent,cursor:"pointer",padding:"8px 0"}}>Show more systems</summary>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginTop:8}}>
            {KNOWN_SYSTEMS.slice(10).map(sys => (
              <button type="button" key={sys.label} onClick={()=>{addFlagship(sys);touch(3);}} className="roi-preset" style={{width:"100%",padding:"10px 12px",textAlign:"left",border:`1px solid ${C.border}`,borderRadius:14,background:"#fff",cursor:"pointer"}}>
                <div style={{fontSize:13,fontWeight:700,color:C.text}}>{sys.label}</div>
                <div style={{fontSize:11,color:C.textMuted,marginTop:1}}>{sys.type} - typical ~{fmtK(sys.cost)}/yr</div>
              </button>
        ...
        <button type="button" onClick={()=>{addFlagship({label:"",cost:250000});touch(3);}} className="roi-link" style={{marginTop:10,border:"none",background:"none",color:C.accent,fontSize:13,fontWeight:700,cursor:"pointer",padding:"8px 0",display:"flex",alignItems:"center",gap:6}}>
          + Add a custom / unlisted system
        </button>
      </div>

      {flagships.length > 0 && (
        <div>
          <div style={{fontSize:13,fontWeight:700,color:C.text,marginBottom:8}}>Your named systems</div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {flagships.map((f, i) => (
              <div key={i} className="roi-flag" style={{padding:"14px 16px",background:C.bg,borderRadius:10,border:`1px solid ${C.borderLight}`}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                  {f.known ? (
                    <div style={{flex:1}}>
                      <div style={{fontSize:14,fontWeight:700,color:C.text}}>{f.known}</div>
                      <div style={{fontSize:11,color:C.textMuted}}>From known systems list</div>
                    </div>
                  ) : (
                    <input type="text" placeholder="System name" value={f.name} onChange={e => updateFlagship(i,"name",e.target.value)}
                      style={{flex:1,padding:"8px 10px",border:`1px solid ${C.borderLight}`,borderRadius:999,fontSize:13,fontFamily:"inherit"}}/>
                  )}
                  <button type="button" onClick={() => removeFlagship(i)} style={{border:"none",background:"none",color:C.textMuted,fontSize:18,cursor:"pointer",padding:"8px",lineHeight:1}} title="Remove">{"\u2715"}</button>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  <span style={{fontSize:12,color:C.textMuted}}>Annual cost:</span>
                  ...
                  <input type="number" value={f.cost} min={0} max={20000000} step={10000} onChange={e => updateFlagship(i,"cost",parseInt(e.target.value)||0)}
                    style={{width:110,padding:"6px 10px",border:`1px solid ${C.borderLight}`,borderRadius:999,fontSize:13,fontFamily:"inherit",textAlign:"right"}}/>
                  <span style={{fontSize:11,color:C.textMuted}}>/yr</span>
                  <label style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:4,fontSize:12,color:C.textMid,cursor:"pointer"}}>
          ...
          <div style={{marginTop:12,padding:"10px 14px",background:C.bg,borderRadius:8,fontSize:12,color:C.textMuted,lineHeight:1.5}}>
            {flagships.length} named system{flagships.length !== 1 ? "s" : ""} totalling {fmtK(flagships.reduce((s,f)=>s+(f.cost||0),0))}/yr.
            {" "}{flagships.filter(f=>f.retire).length} flagged for retirement.
          </div>
        </div>
      )}

      {flagships.length === 0 && (
        <div style={{padding:"20px",textAlign:"center",color:C.textMuted,fontSize:13,background:C.bg,borderRadius:8}}>
          No named systems added yet. Select from the list above, add a custom system, or skip this step to use tier estimates only.
        </div>
      )}
    </Card></div>
    {step<3?<ContinuePrompt onClick={()=>goTo(3,ref3)} label="Next: fine-tune"/>:<LockedConnector/>}

    {/* STEP 4 - Fine-tune */}
    <div ref={ref3} style={{animation:step>=3?"fade-in .4s ease":"none"}}><Card dimmed={step<3}>
      <div style={{fontSize:12,fontWeight:600,color:C.textMuted,textTransform:"uppercase",letterSpacing:".05em",marginBottom:12,display:"flex",alignItems:"center",gap:8}}>
        <span style={{width:24,height:24,borderRadius:"50%",background:step>=3?C.accent:C.border,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700}}>5</span>
        Fine-tune
      </div>
    ...
    {/* CTA */}
    <div ref={refCta} style={{animation:step>=3?"fade-in .4s ease":"none",opacity:step<3?0.3:1,pointerEvents:step<3?"none":"auto",transition:"opacity .4s"}}>
  ...
  const resultsPage = <div>
    ...
    <Card dimmed={dim}><h3 style={{fontSize:16,fontWeight:700,margin:"0 0 4px"}}>Legacy decommission savings</h3>
    ...
    {/* Clinician time */}
    <Card dimmed={dim}><h3 style={{fontSize:16,fontWeight:700,margin:"0 0 4px"}}>{isArchiveOnly?"Legacy lookup time freed":"Clinical workflow time freed"}</h3>
    ...
    <Card dimmed={dim} style={{border:"1px solid #e0c4cf",background:"#faf5f7"}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
        ...
        <h3 style={{fontSize:16,fontWeight:700,margin:0}}>{isArchiveOnly?"Patient safety context":"Patient safety case for migration"}</h3>
    ...
    <Card dimmed={dim} style={{background:C.surface,border:`1px solid ${C.border}`}}>
      <h3 style={{fontSize:16,fontWeight:700,margin:"0 0 4px"}}>Projected KPI improvements</h3>
    ...
    {/* Year-by-year */}
    <Card dimmed={dim}>
      <h3 style={{fontSize:16,fontWeight:700,margin:"0 0 4px"}}>Year-by-year breakdown</h3>
    ...
    {!dim && !leadSubmitted && (
      <Card style={{marginTop:20,background:`linear-gradient(135deg,${C.navy} 0%,${C.navyMid} 100%)`,border:"none",color:"#fff"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
          ...
          <h3 style={{fontSize:18,fontWeight:800,margin:0}}>Get this breakdown in your inbox</h3>
        </div>
        ...
        }} className="roi-cta">{leadSending ? "Generating report..." : "Download PDF report"}</button>
    ...
    {!dim && leadSubmitted && (
      <Card style={{marginTop:20,background:C.greenPale,border:"1px solid #c3ddd3"}}>
        <div style={{textAlign:"center",padding:"12px 0"}}>
          <h3 style={{fontSize:18,fontWeight:800,color:C.green,margin:"0 0 6px"}}>{"✓"} Report downloaded</h3>
    ...
    {!dim && (() => {
      ...
      return <Card style={{marginTop:20}}>
        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
          ...
          <h3 style={{fontSize:17,fontWeight:800,margin:0,color:C.text}}>How we calculated this</h3>
        </div>
        ...
        <S title="4. Clinician time savings">
          ...
          <F>clinicians = beds {"\u00d7"} 2.8 + orgs {"\u00d7"} 80<br/>minsWasted = baseMin {"\u00d7"} dataQuality {"\u00d7"} complexity {"\u00d7"} (1 + switchPenalty)<br/>switchPenalty = max(0, systems - 1) {"\u00d7"} 0.04<br/>hrsSaved = clinicians {"\u00d7"} (minsWasted - residual) {"\u00d7"} 48wks / 60<br/>timeSave = hrsSaved {"\u00d7"} {"\u00a3"}55/hr {"\u00d7"} realisationRate</F>
  ...
  const calculatingPage = <div ref={calcPageRef} style={{display:"flex",alignItems:"center",justifyContent:"center",minHeight:"55vh"}}>
    <div style={{width:"100%",maxWidth:400,textAlign:"center"}}>
      ...
      <h2 style={{fontSize:20,fontWeight:700,color:C.text,marginBottom:4}}>Calculating your ROI</h2>
